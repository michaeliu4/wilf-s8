...
\thanks{Affiliation: Georgia Institute of Technology, Atlanta, Georgia, USA. Correspondence: \texttt{mliu416@gatech.edu}. ORCID: \url{https://orcid.org/0009-0005-3830-8742}.}
...
permutations of length $a+k$ containing $\tau$. We identify the
Ray--West parameter $j(\tau)$, which measures the pattern-dependent
correction to $g_2(\tau)$, as a statistic of the intervals of $\tau$.
The interval formula is derived from Ray and West's classification of two-point
insertions. A cycle basis for a deletion graph gives a second
interpretation: $j(\tau)$ counts excess classes of occurrences of $\tau$
in permutations of length $a+2$, where occurrences are linked by
replacements of one point. The interval formula characterizes the
maximizers of $j$ and shows that, for a uniformly random permutation of length
$n$, $j$ converges in distribution to a Poisson law with mean $2$ as
$n\to\infty$.
We prove that permutations of length eight have exactly $4{,}755$
Wilf classes. The eight symmetries and the known equivalences of
Backelin--West--Xin and Stankova--West generate all identities within
these classes. Exact avoidance counts distinguish the classes through
length $14$, and this cutoff is sharp.
For $g_3$ we give a finite formula for arbitrary patterns and a simple
correction for layered patterns. The formulas characterize the full
Wilf class of the increasing pattern in every length; a comparison with
separated patterns shows cubic pattern dependence in codimension three. An elementary cluster
expansion organizes these counts and gives exact binomial formulas and
Catalan-square corrections for increasing patterns.
\end{abstract}
...
Let $T(\rho)$ be the set of triples of consecutive positions $\{i,i+1,i+2\}$ carrying three consecutive values (a \emph{$3$-interval}) all three of whose pairs are edges of $\Gam$, and let $S(\rho)$ be the set of pairs of disjoint bonds $\{x,x+1\}$, $\{y,y+1\}$ with $x+1<y$ all four of whose cross pairs $\{x,y\},\{x,y+1\},\{x+1,y\},\{x+1,y+1\}$ are edges. Put $t(\rho):=|T(\rho)|$ and $s(\rho):=|S(\rho)|$. Each element of $T(\rho)$ is a triangle of $\Gam$ and each element of $S(\rho)$ is a $4$-cycle of $\Gam$. The two types of cycle are illustrated in \cref{fig:cycles}.
\end{definition}
...
positions in the host. The thick edge is the leading edge, the lexicographically largest
...
\foreach \shift/\panel in {0/{$O=\{2,4,5,6\}$},11/{$O'=\{1,2,3,5\}$}} {
  \begin{scope}[xshift=\shift*.48cm]
  \draw[step=1,black!35,line width=.4pt] (1,1) grid (6,6);
  ...
  \foreach \x/\y in {1/2,3/1}
    \draw[fill=white,line width=.6pt] (\x-.22,\y-.22) rectangle (\x+.22,\y+.22);
  \node[anchor=north] at (3.5,-.6) {\panel};
  \end{scope}
}
...
For the lower bound we compute $|\Av_n(\tau)|$ for $n\le14$ and one representative $\tau$ of each $\equiv_8$-class, using \cref{thm:cluster}: $|\Av_n(\tau)|=n!-\sum_{r=8}^{n}\binom nr^2(n-r)!\,c_r(\tau)$, and $c_r(\tau)$ is the sum of the cluster weights of all clusters of $\tau$ of size $r$. Fix a target codimension $K\ge0$. The length-eight computation below uses cutoffs $K=4,5,6$ adaptively. The program \texttt{clusters.cpp} proceeds in three steps.
...
This paper was developed through continuous collaboration between the author and AI models. The author brought together Vatter's questions on the length-eight Wilf classification, the Ray--West parameter, and codimension three, and required all-length proofs, explicit provenance for each class, and independent verification of the finite results. Claude models (Anthropic), including Fable~5.1, contributed to the classification, structural arguments, certificates, computations, code, and manuscript. GPT-6 Astra Pro (OpenAI) contributed to proof refinement, source checks, code corrections, and independent recounts. The author evaluated the results, shaped their presentation, and takes full responsibility for all mathematical results and the contents of this manuscript.

\begin{thebibliography}{99}

\bibitem{BabsonWest2000}
E.~Babson and J.~West, \emph{The permutations $123p_4\cdots p_m$ and $321p_4\cdots p_m$ are Wilf-equivalent}, Graphs Combin. \textbf{16} (2000), no.~4, 373--380. \url{https://doi.org/10.1007/s003730070001}.

\bibitem{BWX2007}
J.~Backelin, J.~West and G.~Xin, \emph{Wilf-equivalence for singleton classes}, Adv. in Appl. Math. \textbf{38} (2007), no.~2, 133--148. \url{https://doi.org/10.1016/j.aam.2004.11.006}.

\bibitem{BHT2018}
D.~Bevan, C.~Homberger and B.~E. Tenner, \emph{Prolific permutations and permuted packings: downsets containing many large patterns}, J. Combin. Theory Ser. A \textbf{153} (2018), 98--121. Preprint: \href{https://arxiv.org/abs/1608.06931v3}{arXiv:1608.06931v3}. \url{https://doi.org/10.1016/j.jcta.2017.08.006}.
...
\bibitem{BOO2000}
A.~Borodin, A.~Okounkov and G.~Olshanski, \emph{Asymptotics of Plancherel measures for symmetric groups}, J. Amer. Math. Soc. \textbf{13} (2000), no.~3, 481--515. \url{https://doi.org/10.1090/S0894-0347-00-00337-4}.

\bibitem{BHV2008}
R.~Brignall, S.~Huczynska and V.~Vatter, \emph{Simple permutations and algebraic generating functions}, J. Combin. Theory Ser. A \textbf{115} (2008), no.~3, 423--441. \url{https://doi.org/10.1016/j.jcta.2007.06.007}.
...
\bibitem{FRT1954}
J.~S. Frame, G.~de~B. Robinson and R.~M. Thrall, \emph{The hook graphs of the symmetric group}, Canad. J. Math. \textbf{6} (1954), 316--324. \url{https://doi.org/10.4153/CJM-1954-030-1}.

\bibitem{Gessel1990}
I.~M. Gessel, \emph{Symmetric functions and P-recursiveness}, J. Combin. Theory Ser. A \textbf{53} (1990), no.~2, 257--285. \url{https://doi.org/10.1016/0097-3165(90)90060-A}.

\bibitem{GouldenJackson1979}
I.~P. Goulden and D.~M. Jackson, \emph{An inversion theorem for cluster decompositions of sequences with distinguished subsequences}, J. London Math. Soc. (2) \textbf{20} (1979), no.~3, 567--576. \url{https://doi.org/10.1112/jlms/s2-20.3.567}.
...
C.~Homberger, \emph{Counting fixed-length permutation patterns}, Online J. Anal. Comb. \textbf{7} (2012), Paper~3, 12~pp., \url{https://doi.org/10.61091/ojac-703}. Preprint: \href{https://arxiv.org/abs/1211.7117v1}{arXiv:1211.7117v1}.

\bibitem{Johansson2001}
K.~Johansson, \emph{Discrete orthogonal polynomial ensembles and the Plancherel measure}, Ann. of Math. (2) \textbf{153} (2001), no.~1, 259--296. \url{https://doi.org/10.2307/2661375}.

\bibitem{Kaplansky1945}
I.~Kaplansky, \emph{The asymptotic distribution of runs of consecutive elements}, Ann. Math. Statist. \textbf{16} (1945), no.~2, 200--203. \url{https://doi.org/10.1214/aoms/1177731121}.

\bibitem{Krattenthaler2006}
C.~Krattenthaler, \emph{Growth diagrams, and increasing and decreasing chains in fillings of Ferrers shapes}, Adv. in Appl. Math. \textbf{37} (2006), no.~3, 404--431. \url{https://doi.org/10.1016/j.aam.2005.12.006}.
...
\bibitem{Pratt1973}
V.~R. Pratt, \emph{Computing permutations with double-ended queues, parallel stacks and parallel queues}, in: Proceedings of the Fifth Annual ACM Symposium on Theory of Computing (STOC '73), ACM, 1973, pp.~268--277. \url{https://doi.org/10.1145/800125.804058}.

\bibitem{RayWest2003}
N.~Ray and J.~West, \emph{Posets of matrices and permutations with forbidden subsequences}, Ann. Comb. \textbf{7} (2003), no.~1, 55--88. \url{https://doi.org/10.1007/s000260300005}.

\bibitem{Schensted1961}
C.~Schensted, \emph{Longest increasing and decreasing subsequences}, Canad. J. Math. \textbf{13} (1961), 179--191. \url{https://doi.org/10.4153/CJM-1961-015-3}.

\bibitem{Stankova1994}
Z.~E. Stankova, \emph{Forbidden subsequences}, Discrete Math. \textbf{132} (1994), no.~1--3, 291--316. \url{https://doi.org/10.1016/0012-365X(94)90242-9}.

\bibitem{Stankova1996}
Z.~Stankova, \emph{Classification of forbidden subsequences of length 4}, European J. Combin. \textbf{17} (1996), no.~5, 501--517. \url{https://doi.org/10.1006/eujc.1996.0044}.

\bibitem{StankovaWest2002}
Z.~Stankova and J.~West, \emph{A new class of Wilf-equivalent permutations}, J. Algebraic Combin. \textbf{15} (2002), no.~3, 271--290. \url{https://doi.org/10.1023/A:1015016625432}.
...
J.~Wolfowitz, \emph{Note on runs of consecutive elements}, Ann. Math. Statist. \textbf{15} (1944), no.~1, 97--98. \url{https://doi.org/10.1214/aoms/1177731319}.
